...
Релиз скорости взаимодействия — **минорный, обратносовместимый** (только добавления;
существующие инструменты и индексы не меняются).

### Добавлено
- **«Индекс — первым делом, один заход».** Подсказки сервера (`instructions`, <2 КБ) и
  описания инструментов направляют ассистента сразу в `yandex_search_fast` (локальный индекс,
  миллисекунды) на запросы «найди / вспомни / последнее от X», а не в живой обход папок.
  `yandex_find_sender` сужен до разрешения неоднозначного имени.
- **Флаг `read_top` у `yandex_search_fast`.** `read_top=true` дочитывает тело самого свежего
  ПО ДАТЕ совпавшего письма тем же вызовом — схлопывает прежние три живых захода
  (find_sender → search_emails → get_email) в один. Тело проходит тот же защитный путь, что
  `yandex_get_email` (скрытие 2FA-отправителей, маркер недоверенного, обёртка границами,
  provenance, изоляция аккаунта). Если по `from` совпало несколько разных людей — возвращает
  список без тела, чтобы уточнить кого.
- **`alwaysLoad` на горячих инструментах** (`yandex_search_fast`, `yandex_health_check`) — снимает
  задержку «поиска инструмента» Claude Code на первом вызове (безвредно для клиентов без неё).

### Изменено
- **Анти-устаревание для `read_top`.** Перед чтением — дешёвая проверка свежести папки
  (uidValidity / uidNext / exists против индекса): при расхождении свежайшее перерешается вживую
  с проверкой адреса; смена uidValidity без перерешения — отказ от чтения (а не риск чужого
  письма). Результат помечается `index_fresh`.
- **Быстрые таймауты соединения** (`connectionTimeout` 20 с, `greetingTimeout` 10 с) — зависший
  TLS-доступ к Яндексу больше не подвешивает вызов; `socketTimeout` оставлен дефолтным, чтобы не
  рвать длинные стримы индексации.

### Исправлено
- Верхнее письмо в `read_top` больше не выбирается по релевантности — строго по дате.
- Аудит чтения тела через `read_top` пишется как `yandex_get_email` (с message_id), а конверт
  `yandex_search_fast` остаётся без message_id (конвенция non-email-действий).
- Сбой живого дочитывания при `read_top` деградирует (статус), не роняя результат поиска.

567 тестов (561 проходит, 6 unix-only пропускаются на Windows), `npm audit --omit=dev` = 0,
typecheck чисто. Двухпроходное ревью (`/code-review` high + адверсариальная верификация).

## [2.10.0] — 2026-06-23

Релиз удобства и доработок поверх Слоя 3 — **минорный, обратносовместимый** (только
добавления и исправления; ничего ломающего).

### Добавлено
- **Slash-команды — полный MECE-набор из 6.** К `/ymc-config` и `/ymc-update` добавлены
  `/ymc-info` (обзор возможностей по пирамиде Минто, под текущий уровень доступа),
  `/ymc-index` (сборка/обновление/статус локального поискового индекса), `/ymc-trust`
  (доверенные адреса и политика рисков; просмотр свободно, изменения с подтверждением) и
  `/ymc-doctor` (диагностика «почему не работает»: `--check`, версия сервера, разбор
  ошибок). `bin/install-slash.js` сам находит все команды в `.claude/commands/` (без
  захардкоженного списка).
- **MCP-prompt `info`** — тот же обзор возможностей прямо из бандла, для клиентов без
  slash-команд (напр. Claude Desktop).
- **Онбординг** — сообщение установщика, поле `instructions` сервера и README ведут
  нового пользователя к `/ymc-info` первым шагом.

### Исправлено
- **`yandex_get_attachment`: выбор по имени файла больше не мёртв.** Из-за `default(0)`
  у `index` ветка выбора по `filename` была недостижима — теперь `filename` имеет
  приоритет, индекс остаётся запасным (по умолчанию 0).
- **Безопасность: имя сохраняемого вложения очищается от bidi/zero-width.**
  `safeAttachmentFilename` снимал только C0/C1, пропуская RLO (U+202E) и подобное —
  теперь снимает и формат-символы (Trojan-Source / CVE-2021-42574), как путь отображения.
- **`YANDEX_ATTACHMENT_MAX_BYTES=0` действительно блокирует загрузки** (раньше `0` по
  falsy-логике откатывался к лимиту 25 MiB).
- **`has_attachments` и маркер `[attach]` единообразны** на путях списка/поиска/чтения —
  `bodyStructure` запрашивается и там, а не только при индексации.
- **`yandex_unanswered`: меньше ложных «авто-отправителей».** Совпадение
  `newsletter`/`notification` теперь по границам слова, а не как подстрока (не задевает
  живых людей вроде `anna-newsletterova@`).
- **Кэши индекса сбрасываются после `index build`/`update`** (не только `drop`).

### Технические заметки
- 556 тестов (550 проходят, 6 unix-only пропускаются на Windows), `npm audit --omit=dev` = 0.
- Сознательные компромиссы (задокументированы в коде): `yandex_get_attachment` скачивает
  всё письмо ради одного вложения (part-fetch отложен — нужен ручной decode кодировки);
  `size_kb` в каталоге — размер «на проводе» (для base64 ~на треть больше реального).
...
- **`yandex_unanswered` — ранжирование и метки уверенности (option A).** Результаты теперь
  ранжируются по приоритету (PRIMARY priority DESC, SECONDARY date ASC) и размечаются:
  - `tier=reply_likely` — треды с флажком или переписки, в которых владелец уже участвовал
    (всплывают наверх);
  - `tier=fyi_likely` — авто-уведомления и рассылки (noreply, newsletter, digest и т.п.)
    опускаются вниз, но ОСТАЮТСЯ в списке (recall=100%);
  - `tier=normal` — всё остальное.
  Каждый результат дополнен полями `tier` и `reasons[]` с кратким объяснением. Ни одна
  запись не скрывается; чтобы убрать шумного отправителя, используй фильтр `from`.
  Работает полностью метаданными (`flagged`, `seen`, `fromEmail`) — переиндексация не нужна.

### Добавлено

- **`yandex_get_attachment` — новый L0-инструмент (260622-psg).** Скачивает байты
  одного вложения по uid+index и сохраняет в локальный каталог (путь в ответе,
  байты не возвращаются inline и не парсятся — санкционированное явное получение
  одного документа). Защита от path traversal: два слоя — (1) `safeAttachmentFilename`
  приводит имя файла к безопасному basename (убирает `/`, `\\`, `..`, NUL/control,
  ведущие точки; сохраняет кириллицу); (2) проверка вложенности `path.resolve` до
  записи. Cap на размер до записи (YANDEX_ATTACHMENT_MAX_BYTES, default 25 МиБ).
  Права: файл 0o600, каталог 0o700. Конфиг: YANDEX_ATTACHMENT_DIR,
  YANDEX_ATTACHMENT_MAX_BYTES.

- **`yandex_unanswered` — новый L0-инструмент (260622-k20).** Показывает полученные треды,
  в которых последнее письмо пришло от другого человека (не от владельца почты) и старше
  N дней (по умолчанию 7). Треды группируются между папками по графу Message-ID (In-Reply-To)
  и нормализованной теме — ответ в «Отправленных» (в том числе в папках с кириллическими
  именами, напр. «Отправленные») помечает тред как отвеченный без хардкода имён папок;
  классификация производится по адресу отправителя. Работает полностью оффлайн по
  локальному индексу (без живого IMAP). Если индекс не построен — возвращает понятную
  подсказку. Если учётные данные не определены — возвращает `owner_known:false` без ошибки.
  Известное ограничение: рассылки и уведомления, на которые никогда не отвечаешь, попадают
...
[Unreleased]: https://github.com/user/yandex-mail-connector/compare/v2.11.0...HEAD
[2.11.0]: https://github.com/user/yandex-mail-connector/compare/v2.10.0...v2.11.0
[2.10.0]: https://github.com/user/yandex-mail-connector/compare/v2.9.0...v2.10.0
