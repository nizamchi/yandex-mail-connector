...
- **Удалённые письма вычищаются из индекса.** Раньше инкрементальная синхронизация
  ...
  пересобирается. Счётчик писем измеряется, а не вычисляется «на бумаге», и больше
  не уезжает от реальности (накопленный перекос сам исправляется при следующей
  синхронизации).
