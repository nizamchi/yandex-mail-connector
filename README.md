...
[![Версия](https://img.shields.io/badge/версия-2.9.0-blue)](CHANGELOG.md)
[![Тесты](https://img.shields.io/badge/тесты-530_/_0_упавших-success)](#)
...
```bash
npx -y github:nizamchi/yandex-mail-connector#v2.9.0 --check
```
...
Текущая версия — **2.9.0**, июнь 2026 года. Слой 3 «Каталог вложений» (минорный, обратносовместимый релиз — только добавления, старые индексы до-мигрируют сами): коннектор учится находить письма по метаданным вложений (имя файла, тип, отправитель, дата) — **без скачивания тел писем**. По IMAP `BODYSTRUCTURE` (тот же запрос, что и конверты, без лишних обращений к серверу) заполняется `has_attachments` на каждом письме и отдельный манифест `attachments.jsonl` рядом с локальным индексом; поверх него — инструмент `yandex_find_attachments` (фильтры, схлопывание дублей по «имя+размер» с `occurrence_count`, пагинация, ограничение размера выдачи, санитизация имён) и фильтр `has_attachments` в `yandex_search_fast`. Без новых зависимостей. Ship-критерий слоя — `find_attachments(from, mime='application/pdf')` < 50 мс — подтверждён живым прогоном (0.5 мс на реальном ящике). 536 тестов (530 проходят, 6 unix-only пропускаются на Windows), ноль известных уязвимостей (`npm audit --omit=dev`), статическая проверка типов (`npm run typecheck`).
