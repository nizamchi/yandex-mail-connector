...
**Slash-команда для интерактивной смены уровня:** скопировать
`.claude/commands/ymc-config.md` в `~/.claude/commands/` (один раз).
После этого `/ymc-config` в Claude Code открывает пикер со стрелочками —
выбираешь, и команда правит `~/.claude.json`. Перезапуск Claude Code
после смены уровня обязателен — это закладка против TOCTOU-атак
(уровень читается один раз при старте сервера, намеренно). Подробнее
в [INSTALL.md](INSTALL.md#шаг-45-опционально-slash-команда-ymc-config).
