...
«От нуля до паритета», часть 1 — **минорный, обратносовместимый** (только добавления;
существующие инструменты, индексы и пути установки не меняются).

### Добавлено
- **Авто-свежесть индекса (opt-in).** `YANDEX_INDEX_AUTO=on` — при первом старте сервер сам
  собирает индекс (INBOX + Отправленные), затем держит его свежим в фоне (порог
  `YANDEX_INDEX_MAX_AGE_MINUTES`, дефолт 60 мин). **Выключено по умолчанию**; запускается ПОСЛЕ
  подключения и не блокирует инструменты; все сбои не фатальны. Закрывает разрыв «у нового
  пользователя индекс пустой → поиск молчит». Пропущенная при первой сборке папка
  «Отправленные» само-восстанавливается на следующем обновлении.
- **Морфология поиска RU + EN.** Запрос в другой словоформе находит базовую: «выписку» →
  тема «выписка», «invoices» → «invoice». Лёгкий стеммер без новых зависимостей, симметрично
  на индексе и запросе; **без переиндексации** (токены строятся из уже хранимого сырого текста).
- **`capabilities` в `yandex_health_check`** — сообщает реальные возможности сборки
  (`index`, `manifest`, `auto` на L3) для интроспекции агентом (было пусто).

### Исправлено
- **Установка одной командой `npx -y github:...` теперь работает.** Причина падения была не в
  расположении `package.json`, а в системе модулей: CJS-бандл грузился как ESM под корневым
  `type:module` → `require is not defined`. Добавлен маркер `dist/package.json`
  `{"type":"commonjs"}` (статичный + регенерируется при сборке). **Без переноса файлов** —
  пути существующих установок не меняются. Версия корневого `package.json` синхронизирована.

### Безопасность / приватность
- Фоновая авто-свежесть — строго opt-in (требование «никаких фоновых демон-фич без явного
  согласия»). По умолчанию поведение коннектора не меняется.
...
[Unreleased]: https://github.com/user/yandex-mail-connector/compare/v2.12.0...HEAD
[2.12.0]: https://github.com/user/yandex-mail-connector/compare/v2.11.0...v2.12.0
