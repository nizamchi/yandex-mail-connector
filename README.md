...
[![Версия](https://img.shields.io/badge/версия-2.10.0-blue)](CHANGELOG.md)
[![Тесты](https://img.shields.io/badge/тесты-556_/_0_упавших-success)](#)
...
```bash
npx -y github:nizamchi/yandex-mail-connector#v2.10.0 --check
```
...
После сборки появляются инструменты уровня «только чтение»:

- **`yandex_search_fast`** — мгновенный ранжированный поиск по теме и
  отправителю. Поддерживает фильтры `from / since / before / seen / flagged /
  has_attachments` и поиск **только по фильтрам** без текста запроса:
  «непрочитанные за март» → `seen=false`, `since="2025-03-01"`. Редкие, различимые
  слова ранжируются выше частых.
...
- **`yandex_get_attachment`** (L0, только чтение) — сохраняет байты ОДНОГО
  вложения по `uid` + `index` (0-based, из `yandex_get_email`) в локальную
  папку и возвращает путь к файлу. Байты **не** возвращаются inline и **не**
  парсятся (без OCR) — санкционированное явное получение одного документа.
  Конфиг:
  - `YANDEX_ATTACHMENT_DIR` — куда сохранять (default: `<tmpdir>/yandex-mail-mcp-attachments`)
  - `YANDEX_ATTACHMENT_MAX_BYTES` — лимит размера до записи (default: 26214400 = 25 МиБ)
- **`yandex_find_attachments`** (L0, только чтение) — поиск вложений по локальному
  каталогу (метаданные из BODYSTRUCTURE), без скачивания писем и без IMAP. Фильтры
  `from / filename_contains / mime / since / before` (подстрока, без регистра).
  Одинаковые файлы (имя + размер) схлопываются в одну запись с `occurrence_count`
  и списком мест (`folder#uid`) — копию качаешь через `yandex_get_attachment`.
  Каталог по BODYSTRUCTURE считает вложением и встроенные именованные части
  (логотипы) — отсекай их точным `mime` (напр. `application/pdf`). Ship-критерий
  слоя: `find_attachments(from, mime='application/pdf')` < 50 мс (живой прогон — 0.5 мс).

Без индекса инструменты поиска возвращают подсказку «собери индекс» и не мешают
...
Для пользователей Claude Code коннектор поставляется с шестью slash-командами в `.claude/commands/`. Установить один раз:
...
Набор покрывает шесть непересекающихся задач обслуживания (узнать · доступ · индекс · обновить · доверие · диагностика):

| Команда | Что делает |
|---|---|
| `/ymc-info` | Показывает, что коннектор умеет прямо сейчас — под текущий уровень доступа: живой список инструментов по задачам, как устроена безопасность, частые задачи. Читает состояние через `yandex_health_check`, сеть не трогает. |
...
| `/ymc-index` | Управление локальным поисковым индексом (мгновенный поиск + каталог вложений): собрать / обновить / статус / удалить, обёртка над `... index <cmd>`. Перезапуск не нужен — индекс перечитывается на лету. |
...
| `/ymc-trust` | Доверенные адреса для отправки и политика рисков: просмотр свободно, изменения с подтверждением в чате (обёртка над `yandex-mail-mcp-trust`). Доверенные адреса применяются сразу, политика — после перезапуска. |
| `/ymc-doctor` | Диагностика «почему не работает»: гоняет `--check` (связь с IMAP/SMTP по TLS, учётные данные, подписи — без входа в почту), сверяет версию запущенного сервера и разбирает каждую ошибку с шагами починки. |

**Первый шаг после установки — `/ymc-info`.** Он проводит по возможностям под твой
уровень доступа и сам подсказывает, куда дальше: `/ymc-config` — открыть отправку,
`/ymc-index` — собрать индекс для мгновенного поиска, `/ymc-doctor` — если что-то не
работает. В клиентах без slash-команд (например, Claude Desktop) тот же обзор даёт
встроенный prompt `info`.
...
Текущая версия — **2.10.0**, июнь 2026 года — релиз удобства и доработок поверх Слоя 3. Шесть slash-команд закрывают весь цикл обслуживания коннектора (`/ymc-info` — обзор возможностей по пирамиде Минто, `/ymc-config` — уровень доступа, `/ymc-index` — поисковый индекс, `/ymc-update` — обновление, `/ymc-trust` — доверенные адреса, `/ymc-doctor` — диагностика); тот же обзор отдаётся как встроенный MCP-prompt `info` (для клиентов без slash-команд, напр. Claude Desktop). Исправлены ошибки, найденные сквозным код-ревью: выбор вложения по имени в `yandex_get_attachment` (раньше игнорировался), снятие bidi/zero-width символов с имени файла при сохранении на диск (Trojan-Source), честный лимит `YANDEX_ATTACHMENT_MAX_BYTES=0`, единый смысл `has_attachments` и маркера `[attach]` на всех путях (список/поиск/чтение), отсев ложных «авто-отправителей» по границам слова. 556 тестов (550 проходят, 6 unix-only пропускаются на Windows), ноль известных уязвимостей (`npm audit --omit=dev`), статическая проверка типов (`npm run typecheck`).
