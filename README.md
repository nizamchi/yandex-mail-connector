...
Текущая версия — **2.11.0**, июнь 2026 года — релиз скорости взаимодействия. Главная идея: «индекс — первым делом, один заход». Подсказки сервера и описания инструментов направляют ассистента сразу в быстрый поиск по локальному индексу (`yandex_search_fast`), а не в медленный живой обход папок; на «последнее письмо от X» хватает одного вызова. Новый флаг `read_top` у `yandex_search_fast` дочитывает тело самого свежего письма тем же вызовом — раньше это были три медленных захода в почту (найти отправителя → искать → прочитать). Тело идёт через те же защиты, что и обычное чтение (скрытие кодов 2FA, маркер недоверенного отправителя, изоляция аккаунта); при устаревшем индексе перепроверяется вживую и честно помечается. Плюс быстрые таймауты соединения, чтобы зависший доступ к почте не подвешивал вызов. 567 тестов (561 проходит, 6 unix-only пропускаются на Windows), ноль известных уязвимостей (`npm audit --omit=dev`), статическая проверка типов (`npm run typecheck`).
