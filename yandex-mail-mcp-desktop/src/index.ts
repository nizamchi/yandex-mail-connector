...
const SERVER_INSTRUCTIONS = [
  ...
  'SPEED: for find / search / recall ("last email from X", "find mail about Y"),',
  'call yandex_search_fast FIRST -- it answers from an instant local index and',
  'filters by sender substring (from=) across all folders, so "last from X" is',
  'one fast call (add read_top=true to also read that top message in the same',
  'call). Fall back to live yandex_search_emails only if the index is empty.',
  'Run independent read-only lookups in parallel, not one after another.',
...
const server = new McpServer(
  { name: 'yandex-mail-mcp', version: '2.12.0' },
